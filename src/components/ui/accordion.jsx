...
function AccordionItem({ className, ...props }) {
  return (
    <AccordionPrimitive.Item
      ...
      className={cn("border-primary border-b-2 last:border-b-0", className)}
      {...props}
    />
  );
}
...
function AccordionTrigger({ className, children, ...props }) {
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        <ChevronDownIcon className="text-muted-foreground text-primary pointer-events-none size-6 shrink-0 translate-y-0.5 transition-transform duration-200" />
      </AccordionPrimitive.Trigger>
